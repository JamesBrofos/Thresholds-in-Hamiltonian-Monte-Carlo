...
def compute_ks(regex):
    ...
    with tqdm.tqdm(total=num_steps) as pbar:
        for i in range(num_steps):
            ks[i] = spst.kstest(samples[:, i], spst.norm(0.0, 3.0).cdf).statistic
            pbar.update(1)

    return ks


euclid = compute_ks('num-samples-10000-method-euclidean')
rmn = {
    1e-9: compute_ks('thresh-1e-09*num-samples-10000-method-softabs'),
    1e-5: compute_ks('thresh-1e-05*num-samples-10000-method-softabs'),
    1e-1: compute_ks('thresh-0.1*num-samples-10000-method-softabs')
}
