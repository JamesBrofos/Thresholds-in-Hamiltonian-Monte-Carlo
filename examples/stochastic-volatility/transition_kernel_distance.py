...
import hmc
from hmc.applications import stochastic_volatility as distr
...
def main():
    ...
    with open(os.path.join(
            'samples',
            'samples-num-samples-100000-num-burn-10000-thresh-1e-10-max-iters-100-method-riemannian-num-steps-hyper-6-partial-momentum-0.0-newton-True-check-prob-0.01-seed-0.pkl'
    ), 'rb') as f:
    ...
    for j in tqdm.tqdm(range(num_trials)):
        for i, thresh in enumerate(thresholds):
            ...
            ) = distr.hyperparameter_posterior_factory(x, y)
