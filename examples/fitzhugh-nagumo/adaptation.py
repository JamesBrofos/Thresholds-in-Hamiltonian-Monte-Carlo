...
def main():
    ...
    plt.figure()
    plt.plot(loss_hist - 4, label=r'$L_n$')
    plt.plot(loss_avg - 4, label=r'$\bar{L}_n$')
    plt.grid(linestyle=':')
    ...
    plt.figure()
    plt.plot(thresholds, O[:10].T - 4, 'k-', alpha=0.3)
    plt.plot(thresholds, O.mean(0) - 4, linewidth=3, label=r'Expected $L(\delta)$')
    plt.grid(linestyle=':')
