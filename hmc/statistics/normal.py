...
def logpdf(z: np.ndarray, logdet: float, inv: np.ndarray) -> float:
    ...
    Args:
        ...
        inv: The inverse of the covariance matrix of the multivariate normal
            distribution.
    ...
    """
    n = len(z)
    ld = -0.5*n*np.log(2*np.pi) - 0.5*logdet - 0.5*np.matmul(z, inv.dot(z))
    return ld
